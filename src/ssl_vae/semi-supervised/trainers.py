...
class DGMTrainer(VAETrainer):
    ...
    def _calculate_loss(self, x, y=None):
        ...
        # - L(x, y)
        ELBO = log_likelihood + log_prior_y + kl_divergence * self.args["temperature"]
    ...
    def train(self, labelled, unlabelled, n_epochs):
        ...
        :param n_epochs: Number of epochs
        """
        #old_w=list(self.model.parameters())[0].clone()
        for epoch in range(n_epochs):
            for (x, y), (u, _) in zip(cycle(labelled), unlabelled):
                ...
                J.backward()
                #new_w=list(self.model.parameters())[0].clone()
                
                #print([x if x.requires_grad else [] for x in params])
                self.optimizer.step()
                self.optimizer.zero_grad()
