...
named_args.add_argument('--acq_function',
			help="""acq_function for active learning : 'bald', 'random'""",
      default='bald', 
...
# assert args.acq_function in ['bald', 'random']
...
for e in range(Experiments):
  ...
  for i in range(acquisition_iterations):
    ...
    elif args.acq_function == 'random':
      x_pool_index = np.asarray(random.sample(range(0, pool_subset), Queries))
  ...
  np.save(args.save_dir + args.acq_function + "_experiment_" + str(e) +  "_train_loss.npy", all_train_loss)
  np.save(args.save_dir + args.acq_function + "_experiment_" + str(e) +  "_valid_acc.npy", all_valid_acc)
  np.save(args.save_dir + args.acq_function + "_experiment_" + str(e) +  "_test_acc.npy", all_test_acc)
...
np.save(args.save_dir + args.acq_function + "_all_train_loss.npy", Exp_Training_Loss)
np.save(args.save_dir + args.acq_function + "_all_valid_acc.npy", Exp_Valid_Acc)
np.save(args.save_dir + args.acq_function + "_all_test_acc.npy", Exp_Test_Acc)
